...
async fn main() -> Result<()> {
    ...
    #[cfg(not(any(feature = "seal-example", feature = "reef-compute")))]
    ...
    // reef-compute does not use an API key in AppState; the in-enclave TS worker
    // holds its own Seal/Walrus access. Keep AppState's field satisfied.
    #[cfg(feature = "reef-compute")]
    let api_key = String::new();
