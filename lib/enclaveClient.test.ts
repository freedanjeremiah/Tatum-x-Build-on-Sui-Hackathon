...
describe("callEnclave", () => {
  ...
  it("returns the signed payload from process_data, including metricsBytes verbatim", async () => {
    const metricsPayload = JSON.stringify({ n: 5 });
    const metrics_b64 = Buffer.from(metricsPayload).toString("base64");
    globalThis.fetch = vi.fn(async () => new Response(JSON.stringify({
      response: { intent: 0, timestamp_ms: 1717000000000, data: { metrics: { n: 5 }, metrics_b64 } },
    ...
    // metricsBytes must be the EXACT bytes the enclave signed — not a re-encode.
    expect(new TextDecoder().decode(out.metricsBytes)).toBe(metricsPayload);
  });

  it("throws (fail closed) when response is missing metrics_b64", async () => {
    globalThis.fetch = vi.fn(async () => new Response(JSON.stringify({
      response: { intent: 0, timestamp_ms: 1717000000000, data: { metrics: { n: 5 } } },
      signature: "00".repeat(64),
    }), { status: 200 })) as any;
    await expect(callEnclave({ datasetIpId: "0x1", algoHash: "sha256:mean-aggregate" }))
      .rejects.toThrow(/metrics_b64/);
  });
